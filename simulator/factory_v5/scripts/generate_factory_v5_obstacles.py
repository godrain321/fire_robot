#!/usr/bin/env python3
"""Regenerate only factory_v5 obstacles from the current SLAM map artifact."""
...
def write_obstacles(
    ...
    others_records = [
        ...
        if record["surface"] in {"UNMAPPED_SOLID", "NO_GO_WALL"}
    ]
    for record in records:
        if kind == "obstacle" and record["surface"] in {
            "UNMAPPED_SOLID", "NO_GO_WALL"
        }:
            continue
    ...
    for surface, z0, z1, marker in sorted(groups):
        ...
        for rectangle in merge_rectangles(rotated):
            ...
            lines.append(
                f"&OBST ID='V5_{prefix}_{count:04d}', "
    ...
    if kind == "obstacle":
        ...
        # factory_v3 deliberately renders both unknown solids and no-go zones
        # with the OTHERS surface. Preserve that policy in the regenerated map.
        others_mask = clip_to_domain(rotation.mask(source_mask(others_records)))
        ...
        for index, rectangle in enumerate(merge_rectangles(others_mask), start=1):
            gx0, gx1, gy0, gy1 = rectangle
            lines.append(
                f"&OBST ID='V5_OTHERS_{index:04d}', "
                f"XB={gx0 * RESOLUTION:.3f},{gx1 * RESOLUTION:.3f},"
                f"{gy0 * RESOLUTION:.3f},{gy1 * RESOLUTION:.3f},"
...
def write_scenario(path: Path, rotation: Rotation) -> tuple[int, int]:
    ...
    for (surface, z0, z1), group in sorted(groups.items()):
        for rectangle in merge_rectangles(
            ...
            gx0, gx1, gy0, gy1 = rectangle
            lines.append(
                f"&OBST ID='V5_CARDBOARD_{scenario_count:03d}', "
                f"XB={gx0 * RESOLUTION:.3f},{gx1 * RESOLUTION:.3f},"
                f"{gy0 * RESOLUTION:.3f},{gy1 * RESOLUTION:.3f},"
                f"{z0:.3f},{z1:.3f}, SURF_ID='{surface}' /"
            )
    ...
    if oil_records:
        ...
        for rectangle in merge_rectangles(oil_mask):
            ...
            gx0, gx1, gy0, gy1 = rectangle
            lines.append(
                f"&VENT ID='V5_OIL_SPILL_{oil_count:03d}', "
                f"XB={gx0 * RESOLUTION:.3f},{gx1 * RESOLUTION:.3f},"
    ...
    for index, rectangle in enumerate(
        ...
        gx0, gx1, gy0, gy1 = rectangle
        lines.append(
            f"&VENT ID='V5_ROOF_VENT_{index:03d}', "
            f"XB={gx0 * RESOLUTION:.3f},{gx1 * RESOLUTION:.3f},"
            f"{gy0 * RESOLUTION:.3f},{gy1 * RESOLUTION:.3f},"
...
def write_semantics(rotation: Rotation) -> None:
    ...
    for section in ("poses", "landmarks"):
        for item in source.get(section, {}).values():
            ...
            item["fds_v3"] = rotated
    (V5_DIR / "config" / "semantic_points.yaml").write_text(
...
def write_fds(rotation: Rotation) -> None:
    ...
    text = f"""&HEAD CHID='factory_v5', TITLE='Rotated factory_v2; EXIT1 aligned to X axis' /
...
&TAIL /
"""
    (V5_DIR / "factory_v5.fds").write_text(text, encoding="utf-8")
...
def write_overlay(source_mask_all: np.ndarray, rotated_mask: np.ndarray, theta: float) -> None:
    ...
    labels = (
        ...
        f"factory_v5 rotated ({math.degrees(theta):.3f} deg)",
    )
    ...
    cv2.imwrite(
        str(V5_DIR / "validation" / "rotation_overlay.png"),
        cv2.hconcat(padded),
    )


def main() -> int:
    """Rotate the map-derived obstacle artifact without changing v3 settings."""
    source_obstacles = V5_DIR / "generated" / "source_obstacles_unrotated.inc"
    reference_exits = V2_DIR / "generated" / "exits.inc"
    protected_paths = {
        "exits": V5_DIR / "generated" / "exits.inc",
        "scenario": V5_DIR / "generated" / "scenario.inc",
        "exit_blockers": V5_DIR / "config" / "exit_blockers.inc",
    }
    before = {name: sha256(path) for name, path in protected_paths.items()}
    obstacle_records = parse_records(source_obstacles, OBST_RE)
    exit_records = parse_records(reference_exits, OBST_RE)
    ...
    obstacle_count, source_rotated, others_mask = write_obstacles(
        V5_DIR / "generated" / "obstacles.inc", obstacle_records, rotation, "obstacle",
        enclosure_records=exit_records,
    )
    after = {name: sha256(path) for name, path in protected_paths.items()}
    if after != before:
        raise RuntimeError("a protected factory_v3-equivalent setting changed")

    report = {
        "status": "PASS",
        "protected_settings_unchanged": True,
        "protected_setting_hashes": after,
        "source_obstacles_sha256": sha256(source_obstacles),
    ...
    (V5_DIR / "validation" / "obstacle_generation_report.json").write_text(
