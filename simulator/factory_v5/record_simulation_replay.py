#!/usr/bin/env python3
"""Record factory_v5 once, then replay its GUI without loading FDS/NPZ data.
...
DEFAULT_VIDEO = BASE / "output/factory_v5_replay.mp4"
DEFAULT_DATA = BASE / "output/factory_v5_replay.pkl.gz"
...
def read_replay_info(path: Path) -> tuple[dict, int, float | None, float | None]:
    ...
    with gzip.open(path, "rb") as stream:
        ...
        if header.get("format") != "factory_v5_gui_replay":
            raise ValueError("not a factory_v5 replay sidecar")
        while True:
            try:
