...
def _read_fds_source_tree(fds_path: Path, seen: set[Path] | None = None) -> str:
    """Read an FDS file and recursively append CATF geometry sources."""
    resolved = fds_path.resolve()
    seen = set() if seen is None else seen
    if resolved in seen:
        raise ValueError(f"recursive CATF include detected: {resolved}")
    seen.add(resolved)
    ...
    for match in re.finditer(
        ...
        body = match.group(1)
        files_match = re.search(r"\bOTHER_FILES\s*=\s*(.*)", body, flags=re.I | re.S)
        if files_match is None:
            continue
        for relative_name in re.findall(r"['\"]([^'\"]+)['\"]", files_match.group(1)):
            child = (resolved.parent / relative_name).resolve()
            if not child.is_file():
                raise FileNotFoundError(f"CATF source not found: {child}")
            included.append(_read_fds_source_tree(child, seen))
    seen.remove(resolved)
    return text + "\n" + "\n".join(included)


def _read_fds_namelists(fds_path: Path) -> list[tuple[str, str]]:
    text = _read_fds_source_tree(fds_path)
    cleaned = []
...
def load_factory_geometry(
    ...
    """Load mesh, OBST and HOLE geometry, including CATF source files."""
